...
const getSankeyData = createServerFn({ method: 'GET' }).handler(async () => {
    ...
    try {
        ...
        const nodesMap = new Map<string, { id: string; addr: string; day: string; label: string }>()
...
export const Route = createFileRoute('/holders')({
    loader: async () => {
        return { holderSeries: await getHolderSeries(), sankeyData: await getSankeyData() }
    },
    component: Holders,
})
...
function Holders() {
    ...
    const handleCopyAddress = useCallback((addr: string) => {
        if (navigator?.clipboard?.writeText) {
            navigator.clipboard.writeText(addr).catch((err) => console.warn('Copy failed', err))
        }
    }, [])
        ...
        <div className="min-h-screen bg-slate-50 text-slate-900 font-sans">
            ...
            <section className="mx-auto flex max-w-5xl flex-col gap-3 px-3 pb-8">
                ...
                {(() => {
                    ...
                    const grouped = d3.group(data.sankeyData.links, linkDay as (l: SankeyLink) => string) as Map<
                        string,
                        SankeyLink[]
                    >
                    const days = Array.from(grouped.keys()) as string[]
                        ...
                        <div className="flex flex-col gap-4">
                            {days.map((day: string) => {
                                ...
                                const graph = sankeyGen({
                                    nodes: dayNodes.map((n: SankeyNode) => ({ ...n })),
                                    links: dayLinks.map((l: SankeyLink) => ({ ...l })),
                                }) as any
                                            ...
                                            <g>
                                                ...
                                                {(graph.nodes as any[]).map((node) => (
                                                    <g
                                                        ...
                                                        onClick={() => handleCopyAddress(node.addr)}
                                                        role="button"
                                                        tabIndex={0}
                                                        onKeyDown={(e) => {
                                                            if (e.key === 'Enter' || e.key === ' ') {
                                                                e.preventDefault()
                                                                handleCopyAddress(node.addr)
                                                            }
                                                        }}
                                                        style={{ cursor: 'pointer' }}
                                                        aria-label={`Copy ${node.addr} to clipboard`}
                                                    >
                                                        <rect
